...
- **`## priorities` items MUST be plain `- ` bullets, never `- [ ]` checkboxes.** AGENTS.md §4.7: an external TODO pipeline reads daily-note checkboxes, so auto-emitted ones pollute the user's task list. The user adds checkboxes manually (or explicitly asks an agent to). This applies to all sub-items as well.
